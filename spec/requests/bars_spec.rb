...
RSpec.describe 'GET /bars/nearest', type: :request do
  ...
  before do
    ...
    allow(finder).to receive(:call).and_return([
      { id: 1, name: 'Near Bar', x: 1.0, y: 1.0, distance: 1.4142 }
    ])
  end
  ...
  context 'with valid params' do
    ...
    before { get '/bars/nearest', params: params }
  ...
  context 'with missing required params' do
    it 'returns 400 when x is missing' do
      get '/bars/nearest', params: { y: '2.0' }
      expect(response).to have_http_status(:bad_request)
    end

    it 'returns 400 when y is missing' do
      get '/bars/nearest', params: { x: '1.0' }
      expect(response).to have_http_status(:bad_request)
    end
  ...
  context 'when a database error occurs' do
    before do
      ...
      get '/bars/nearest', params: { x: '1.0', y: '2.0' }
    end
  ...
  context 'with invalid param types' do
    it 'returns 400 when x is not a number' do
      get '/bars/nearest', params: { x: 'abc', y: '2.0' }
      expect(response).to have_http_status(:bad_request)
    end

    it 'returns 400 when y is not a number' do
      get '/bars/nearest', params: { x: '1.0', y: 'abc' }
      expect(response).to have_http_status(:bad_request)
    end

    it 'returns 400 when limit is not an integer' do
      get '/bars/nearest', params: { x: '1.0', y: '2.0', limit: 'abc' }
      expect(response).to have_http_status(:bad_request)
    end

    it 'returns 400 when limit is zero' do
      get '/bars/nearest', params: { x: '1.0', y: '2.0', limit: '0' }
      expect(response).to have_http_status(:bad_request)
    end

    it 'returns 400 when limit is negative' do
      get '/bars/nearest', params: { x: '1.0', y: '2.0', limit: '-1' }
      expect(response).to have_http_status(:bad_request)
    end
  end
end
